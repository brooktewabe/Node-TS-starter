import { type MongooseBulkWriteOptions, type PopulateOptions } from 'mongoose'

import permissionGroupModel from '../models/permissionGroup.model.ts'
import PermissionModel from '../models/permission.model.ts'
import type {  PermissionGroupUpdate,  IPermissionGroup,  PermissionGroupFilter,  PermissionGroupOptions,  PermissionGroupProjection } from '../config/types/permission_group.js'
...
interface PermissionGroupServiceParams {
  method:
    | "create"
    | "get"
    | "get collection"
...
interface PermissionGroupReturn {
  statusCode: number
  body: { error: unknown
    data?: IPermissionGroup | IPermissionGroup[] | null
    //  | PaginateResult<any>
      | BulkWriteResult 
...
/**
 * PermissionGroupService function
 */
export async function permissionGroupService (props: PermissionGroupServiceParams): Promise<any> /*Promise<PermissionGroupReturn | PermissionGroupReturns>*/ {
  switch (props.method) {
    case "create": {
      const { data } = props;
