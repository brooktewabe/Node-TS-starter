...
import type {
   ...
   PermissionOptions,
   BulkWritePermission,
} from "../config/types/permission.js";
 ...
 */
export async function permissionService(
  props: PermissionServiceParams
): Promise<PermissionReturn> {
  switch (props.method) {
    case "create": {
      const { data } = props;
...
async function insertManyPermission(
  arrayOfPermissions: IPermission[]
): Promise<PermissionReturn> {
  try {
    const insertManyResult = await permissionModel.insertMany(
      arrayOfPermissions
    );
